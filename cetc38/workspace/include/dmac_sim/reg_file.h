...
//zqw+ 2019.1.11 SRIO相关寄存器
	//写DMA操作
	RegType RAB_WDMA_N_CTRL_R[SRIO_NUM];			// 写DMA控制寄存器  0x20800
	RegType RAB_WDMA_N_ADDR_R[SRIO_NUM];            // 写DMA描述符地址寄存器 0x20804
	RegType RAB_WDMA_N_STAT_R[SRIO_NUM];            // 写DMA状态寄存器 0x20808
	RegType RAB_WDMA_N_ADDR_EXT_R[SRIO_NUM];        // 写DMA高位地址寄存器 0x2080C
	//读DMA操作
	RegType RAB_RDMA_N_CTRL_R[SRIO_NUM];			// 读DMA控制寄存器  0x20880
	RegType RAB_RDMA_N_ADDR_R[SRIO_NUM];            // 读DMA描述符地址寄存器 0x20884
	RegType RAB_RDMA_N_STAT_R[SRIO_NUM];            // 读DMA状态寄存器 0x20888
	RegType RAB_RDMA_N_ADDR_EXT_R[SRIO_NUM];        // 读DMA高位地址寄存器 0x2088C
	//DMA描述符
	RegType RAB_DMA_IADDR_DESC_SEL_R[SRIO_NUM];	        // DMA描述符选择寄存器  0x20900
	RegType RAB_DMA_IADDR_DESC_CTRL_R[SRIO_NUM];	    // DMA描述符寄存器-描述符控制寄存器  0x20904
	RegType RAB_DMA_IADDR_DESC_SRC_ADDR_R[SRIO_NUM];    // DMA描述符寄存器-源地址寄存器      0x20908
	RegType RAB_DMA_IADDR_DESC_DEST_ADDR_R[SRIO_NUM];   // DMA描述符寄存器-目的地址寄存器    0x2090C
	RegType RAB_DMA_IADDR_DESC_NEXT_ADDR_R[SRIO_NUM];   // DMA描述符寄存器-下一指针地址寄存器  0x209010
	
	//zqw+ 2019.2.25 主设备ID寄存器、页号
	RegType Base_Device_ID_CSR_R[SRIO_NUM];             //主设备ID寄存器，用于存放Source ID
	RegType RAB_APB_CSR_R[SRIO_NUM];                    //RAB_APB_CSR Register，APB控制状态寄存器  0x20030  用于存放页号
