...
//zqw+ 2019.3.19 分包、形成SRIO帧函数
	unsigned int srio_wr_len;
	vector<unsigned int> srio_wr_len_Buffer;
	unsigned int srio_rd_len;
	bool wr_first_frame_flag;
	vector<bool> wr_first_frame_flag_Buffer;
	bool rd_first_frame_flag;
	DSPU8 Transaction_ID;
	void SRIOBreakStream(unsigned int CH_ID,DMAC_CMD CMD);
	void SRIOFormFrame(unsigned int CH_ID,DMAC_CMD CMD,DMAC_MEM * ptr);
//zqw+ 2019.3.25
	void SRIOFormDMAC(SRIO_FRAME * srio_frame_msg, DMAC_MEM * dmac_msg,unsigned int ch_id);
