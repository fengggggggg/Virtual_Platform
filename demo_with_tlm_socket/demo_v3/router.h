...
SC_MODULE(Router)
{
	...
	simple_target_socket<Router>				targ_socket[N_INIT];
	simple_initiator_socket_tagged<Router>*		init_socket[N_TARG];
											...
											tlm_phase &phase, sc_time& delay )
	{
		...
		targ_socket[0]->nb_transport_bw( trans, phase, delay );
		targ_socket[1]->nb_transport_bw(trans, phase, delay);
		return tlm::TLM_ACCEPTED;
	}
